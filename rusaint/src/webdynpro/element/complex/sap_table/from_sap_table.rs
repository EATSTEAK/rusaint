...
// TODO: Extract duplicate code fragments for textise elements
impl<'body> FromSapTable<'body> for Vec<Option<String>> {
    fn from_table(
        ...
        let vec = iter
            .map(|val| match val {
                Ok(cell) => match cell.content(parser) {
...
impl<'body> FromSapTable<'body> for Vec<String> {
    fn from_table(
        ...
        iter.map(|val| match val {
            Ok(cell) => match cell.content(parser) {
                Some(wrapper) => Ok(ElementWrapper::from_def(&wrapper, parser)?.textise(parser)?),
    ...
    fn from_table(
        ...
        let header_string = header_iter
            .map(|val| match val {
                Ok(cell) => match cell.content(parser) {
                    Some(wrapper) => {
                        Ok(ElementWrapper::from_def(&wrapper, parser)?.textise(parser)?)
                    }
