...
impl<'a> StudentTeachingMajorInformation {
    ...
    pub(crate) fn with_parser(
        ...
        Ok(Self {
            major_name: parser.read(ReadInputFieldValueCommand::new(Self::MAJOR_OTYPE))
                .ok(),
            qualification_number: parser.read(ReadInputFieldValueCommand::new(Self::MAJOR_QUAL_NUM))
                .ok(),
...
impl<'a> StudentLifelongInformation {
    ...
    pub(crate) fn with_parser(parser: &'a ElementParser) -> Result<StudentLifelongInformation, WebDynproError> {
        Ok(Self {
            apply_date: parser
