...
use crate::webdynpro::{application::BasicApplication, element::{client_inspector::ClientInspector, loading_placeholder::LoadingPlaceholder, form::Form, custom::{Custom, CustomClientInfo}, ElementDef}, error::ClientError};
...
impl BasicUSaintApplication {
    pub const CLIENT_INSPECTOR_WD01: ElementDef<'_, ClientInspector<'_>> = ElementDef::new("WD01");
    pub const CUSTOM: Custom<'_> = Custom::new("WD01");
    pub const CLIENT_INSPECTOR_WD02: ElementDef<'_, ClientInspector<'_>> = ElementDef::new("WD02");
    pub const LOADING_PLACEHOLDER: ElementDef<'_, LoadingPlaceholder<'_>> = ElementDef::new("_loadingPlaceholder_");

    pub async fn new(app_name: &str) -> Result<BasicUSaintApplication, ClientError> {
        Ok(BasicUSaintApplication(BasicApplication::new(SSU_WEBDYNPRO_BASE_URL, app_name).await?))
    }

    pub async fn load_placeholder(&mut self) -> Result<(), ClientError> {
        let body = self.body();
        let wd01 = Self::CLIENT_INSPECTOR_WD01.elem(body)?;
        let wd02 = Self::CLIENT_INSPECTOR_WD02.elem(body)?;
        let load_ph = Self::LOADING_PLACEHOLDER.elem(body)?;
