...
- 테스트를 실행하기 전에, `cargo run -p rusaint-cli -- create-session -o session.json`을 실행하여 세션을 파일로 생성하여야 합니다 (사용하는 쉘에 따라 정확한 명령어는 달라질 수 있습니다).
...
```kotlin
dependencies {
  implementation("dev.eatsteak:rusaint:0.15.0")
}
```
